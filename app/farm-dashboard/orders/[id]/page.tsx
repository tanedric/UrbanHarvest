...
export default function OrderDetail() {
  ...
  const { reviews, addReview, checkForUpdates: checkReviewUpdates } = useReviewStore()
  ...
  useEffect(() => {
    ...
    // Scroll to review section if URL has #review hash
    if (window.location.hash === "#review" && reviewSectionRef.current) {
      setTimeout(() => {
  ...
  }, [isAuthenticated, orderId, orders, router, toast, checkForUpdates, checkReviewUpdates])
    ...
    <div className="flex flex-col min-h-screen">
      <main className="flex-1 container mx-auto px-4 py-12">
        ...
        {order.status === "delivered" && !hasReviewed && (
          <Card ref={reviewSectionRef} id="review">
            <CardHeader>
